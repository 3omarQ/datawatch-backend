...
(function () {
  ...
  function clearHover() {
    if (hoverEl && !hoverEl.classList.contains('__f')) hoverEl.classList.remove('__h');
    hoverEl = null;
  }
  ...
  document.addEventListener('mousemove', function (e) {
    ...
    clearHover();
    hoverEl = el;
    if (!el.classList.contains('__f')) el.classList.add('__h');
  ...
  window.addEventListener('message', function (e) {
    ...
    if (e.data.type === 'EP_RESET') {
      fieldSelectors = [];
      qs('.__f, .__h').forEach(function (n) { n.classList.remove('__f', '__h'); });
      hoverEl = null;
    }
  });
})();
